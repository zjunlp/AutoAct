...
from benchmark_run.pre_prompt import (
    react_agent_prompt,
    zeroshot_agent_prompt_hotpotqa,
    zeroshot_agent_prompt_scienceqa,
    plan_prompt,
    planner_agent_prompt,
    plannerreact_agent_prompt
)
from benchmark_run.fewshots import REACT_EXAMPLE, PLANNER_EXAMPLE, PLAN_EXAMPLE, PLANNERREACT_EXAMPLE

token_enc = tiktoken.get_encoding("cl100k_base")
from benchmark_run.utils import call_bing_search, parse_bing_result
...
class BaseAgent:
    ...
    def prompt_agent(self,agent_type) -> str:
        if agent_type == 'plan':
            generation = self.llm_plan(self._build_agent_prompt())
        elif agent_type == 'action':
            generation = self.llm_action(self._build_agent_prompt())
        elif agent_type == 'reflect':
            generation = self.llm_reflect(self._build_agent_prompt())
    ...
    def _think(self):
        self.scratchpad += f'\nThought {self.step_n}: '
        if self.answer != '':
            thought = self.prompt_agent(agent_type='reflect')
        else:
            thought = self.prompt_agent(agent_type="plan")
        self.scratchpad += thought
        print(self.scratchpad.split('\n')[-1])
    
    def _action(self):
        self.scratchpad += f'\nAction {self.step_n}:'
        action_type = self.prompt_agent(agent_type='plan')
        self.scratchpad += f'{action_type}'
        if self.answer != '':
            action_argument = self.prompt_agent(agent_type='reflect')
        else:
            action_argument = self.prompt_agent(agent_type='action')
        self.scratchpad += f'[{action_argument}]'
        print(self.scratchpad.split('\n')[-1])
        return action_type, action_argument
...
class MetaZeroshotThinkAgentHotPotQA(BaseAgent):
    def __init__(self,
        question: str,
        key: str,
        llm_plan: BaseLLM,
        ...
        super().__init__(question, key, llm_plan, llm_action, llm_reflect, context_len)

        self.examples = ""
        self.agent_prompt = zeroshot_agent_prompt_hotpotqa
    ...
    def _action(self):
        self.scratchpad += f'\nAction: '
        if self.last_step == "Finish":
            action_res = self.prompt_agent(agent_type='reflect')
            action_type = action_res[:action_res.find('[')]
            action_argument = action_res[action_res.find('[')+1:action_res.find(']')]
            self.scratchpad += action_res
        else:
            action_type = self.prompt_agent(agent_type='plan')
            self.scratchpad += f'{action_type}'
            action_argument = self.prompt_agent(agent_type='action')
    ...
    def step(self) -> None:
        ...
        if action_type == 'Finish':
            ...
            if self.last_step == "Retrieve":
                ...
                try:
                    self.scratchpad += format_step(self.bing_lookup(argument))
                except ValueError:
                    self.scratchpad += f'The last page Searched was not found, so you cannot Lookup a keyword in it. Please try one of the similar pages given.'
            else:
                self.scratchpad += "Lookup must after Retrive or BingSearch"
        #add Bing_search
        elif action_type == 'BingSearch':
            try:
            ...
            self.scratchpad += 'Invalid Action. Valid Actions are Lookup[<topic>] Retrieve[<topic>] BingSearch[<topic>] Finish[<answer>] and Reflect[right/wrong].'
            # self.scratchpad += 'Invalid Action. You can only generate Lookup[<topic>], Search[<topic>] or Finish[<answer>].'
    ...
    def _build_agent_prompt(self) -> str:
        self.tools = ""
        with open("/data/rolnan/AutoAct/Self_Plan/Tool_Selection/HotpotQA_Tools.json") as f:
            tool_selected = json.load(f)
            for index,tool in enumerate(tool_selected):
                usage = tool["usage"]
                self.tools += f"({index+1}) {usage}\n"
...
class MetaZeroshotThinkAgentScienceQA(BaseAgent):
    def __init__(self,
        question: str,
        choices: list,
        key: str,
        ...
        self.examples = ""
        self.agent_prompt = zeroshot_agent_prompt_scienceqa
        self.name = "ZeroshotThink_ScienceQA_run_Agent"
        self.last_step = ""
        self.bingresults = []
        self.bingindex = 0
        self.bingkeyword = ""
...
def get_agent(agent_name):
    if agent_name in ["ZeroshotThink_HotPotQA_run_Agent"]:
        return MetaZeroshotThinkAgentHotPotQA
    if agent_name in ["ZeroshotThink_ScienceQA_run_Agent"]:
        return MetaZeroshotThinkAgentScienceQA