...
from benchmark_run.pre_prompt import (react_agent_prompt, zeroshot_agent_prompt_hotpotqa,zeroshot_agent_prompt_scienceqa, 
...
class BaseAgent:
    ...
    def step(self) -> None:
        ...
        if action_type == 'Retrieve':
            ...
            try:
                responses = call_bing_search("https://api.bing.microsoft.com/v7.0/search","5fc96b4311e74e519dae3c2acfa56ef8",argument,count=4)
...
class ZeroshotThinkAgentHotpotQA(BaseAgent):
    def __init__(self,
                 question: str,
                 key: str,
                 llm,
    ...
    def step(self) -> None:
        ...
        if action_type == 'Finish':
            ...
            elif self.last_search == "BingSearch":
                try:
                    self.scratchpad += format_step(self.bing_lookup(argument))
                except ValueError:
                    self.scratchpad += f'The last page Searched was not found, so you cannot Lookup a keyword in it. Please try one of the similar pages given.'
            else:
                self.scratchpad += "Invalid action. Lookup only follows the Retrieve or BingSearch action."
                
        #add Bing_search
        elif action_type == 'BingSearch':
            self.last_search = "BingSearch"
            try:
...
class ZeroshotThinkAgentScienceQA(BaseAgent):
    def __init__(self,
                 question: str,
                 key: str,
                 llm,
    ...
    def _build_agent_prompt(self) -> str:
        self.tools = ""
        with open("/data/rolnan/AutoAct/Self_Plan/Tool_Selection/ScienceQA_Tools.json") as f:
            tool_selected = json.load(f)
            for index,tool in enumerate(tool_selected):
                usage = tool["usage"]
                self.tools += f"({index+1}) {usage}\n"
...
def get_agent(agent_name):
    if agent_name in ["ZeroshotThink_HotPotQA_run_Agent"]:
        return ZeroshotThinkAgentHotpotQA
    if agent_name in ["ZeroshotThink_ScienceQA_run_Agent"]:
        return ZeroshotThinkAgentScienceQA