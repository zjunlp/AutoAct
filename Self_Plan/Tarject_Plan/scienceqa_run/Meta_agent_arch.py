...
import numpy as np
from scienceqa_run.pre_prompt import (
    react_agent_prompt,
    zeroshot_agent_prompt, 
    plan_prompt,
...
class MetaZeroshotThinkAgent(BaseAgent):
    def __init__(self,
        question: str,
        choices: list,
        key: str,
...
class MetaZeroshotThinkAgentNoReflect(BaseAgent):
    def __init__(self,
        question: str,
        choices: list,
        key: str,
...
class MetaRowZeroshotThinkAgent(BaseAgent):
    def __init__(self,
        question: str,
        key: str,
        llm_plan: BaseLLM,
    ...
    def _action(self):
        ...
        if self.last_step == "Finish":
            ...
            action_res = self.prompt_agent(agent_type='action')
        action_type = action_res[:action_res.find('[')]
        action_argument = action_res[action_res.find('[')+1:action_res.find(']')]
        self.scratchpad += action_res
        print(self.scratchpad.split('\n')[-1])
...
def get_agent(agent_name):
    ...
    if agent_name in ["ZeroshotThink_HotPotQA_run_Agent_NoReflect"]:
        return MetaZeroshotThinkAgentNoReflect